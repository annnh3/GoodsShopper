...
namespace GoodsShopper.RelayServer.ActionHandler
{
    public class CategoryActionHandler : IActionHandler
    {
        ...
        public override bool Execute(ActionModule actionModule)
        {
            try
            {
                var action = JsonConvert.DeserializeObject<CategoryAction>(actionModule.Message);

                cacheSvc.Upsert(new[] { action.Categories });
                return true;
            }
            catch (Exception ex)
            {
