...
{
    public class ProductTCache : ICache
    {
        /// <summary>
        /// 物件鎖
        /// </summary>
        ...
        public ProductTCache(MemoryCache cache) : base(cache)
        {
        ...
        public override void Delete<T>(Func<T, bool> expression)
        {
            ...
            lock (_lock)
            {
                var legacyData = GetData<IEnumerable<ProductT>>();
                UpdateData(legacyData.Where(p =>
                    !legacyData.Where(expression as Func<ProductT, bool>)
        ...
        {
            return typeof(T) != typeof(ProductT)
                ? new List<T>().FirstOrDefault()
                : (GetData<IEnumerable<ProductT>>().Where(expression as Func<ProductT, bool>) as IEnumerable<T>).FirstOrDefault();
        }
        ...
        public override IEnumerable<T> Get<T>(Func<T, bool> expression)
        {
            return typeof(T) != typeof(ProductT)
                ? new List<T>()
                : GetData<IEnumerable<ProductT>>().Where(expression as Func<ProductT, bool>) as IEnumerable<T>;
        }
        ...
        public override void Upsert<T>(IEnumerable<T> records)
        {
            ...
            var updateData = records as IEnumerable<ProductT>;
