...
{
    public class ProductInfoCache : ICache
    {
        /// <summary>
        /// 物件鎖
        /// </summary>
        ...
        public ProductInfoCache(MemoryCache cache) : base(cache)
        {
        ...
        {
            return typeof(T) != typeof(ProductInfo)
                ? new List<T>().FirstOrDefault()
                : (GetData<IEnumerable<ProductInfo>>().Where(expression as Func<ProductInfo, bool>) as IEnumerable<T>).FirstOrDefault();
        }
        ...
        public override IEnumerable<T> Get<T>(Func<T, bool> expression)
        {
            return typeof(T) != typeof(ProductInfo)
                ? new List<T>()
                : GetData<IEnumerable<ProductInfo>>().Where(expression as Func<ProductInfo, bool>) as IEnumerable<T>;
        }
        ...
        public override void Initialize()
        {
            ...
            lock (_lock)
            {
                UpdateData(new List<ProductInfo>());
            }
        }
        ...
        public override void Upsert<T>(IEnumerable<T> records)
        {
            ...
            var updateData = records as IEnumerable<ProductInfo>;
