...
namespace GoodsShopper.RelayServer.Domain.Cache
{
    public class CategoryCache : ICache
    {
        ...
        public CategoryCache(MemoryCache cache) : base(cache)
        {
        ...
        {
            return typeof(T) != typeof(Category)
                ? new List<T>().FirstOrDefault()
                : (GetData<IEnumerable<Category>>().Where(expression as Func<Category, bool>) as IEnumerable<T>).FirstOrDefault();
        }
        ...
        public override IEnumerable<T> Get<T>(Func<T, bool> expression)
        {
            return typeof(T) != typeof(Category)
                ? new List<T>()
                : GetData<IEnumerable<Category>>().Where(expression as Func<Category, bool>) as IEnumerable<T>;
        }
        ...
        public override void Upsert<T>(IEnumerable<T> records)
        {
            ...
            var updateData = records as IEnumerable<Category>;

            lock (_lock)
            {
                var legacyData = GetData<IEnumerable<Category>>();
