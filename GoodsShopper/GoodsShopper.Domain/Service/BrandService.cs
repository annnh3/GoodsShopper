...
namespace GoodsShopper.Domain.Service
{
    public class BrandService : IBrandService
    {
        ...
        public BrandService(string serviceUri, int timeout = 5)
        {
            this.route = $"{serviceUri}/api/Brand";
            this.timeout = timeout * 1000;
        }

        public (Exception exception, Brand brand) Insert(BrandInsertDto brand)
        {
            try
            {
                ...
                using (var br = new BinaryReader(response.GetResponseStream()))
                {
                    ...
                    var result = Brand.ToConstruct(bytes);
                    return (null, result);
                }
            }
            catch (Exception ex)
        ...
        public (Exception exception, BrandQueryResponseDto response) Query(BrandQueryDto request)
        {
            try
            {
                ...
                {
                    byte[] bytes = br.ReadBytes((int)response.ContentLength);
                    var result = BrandQueryResponseDto.ToConstruct(bytes);
                    return (null, result);
                }
            }
            catch (Exception ex)
