...
namespace GoodsShopper.Domain.Service
{
    public class ProductService : IProductService
    {
        ...
        public ProductService(string serviceUri, int timeout = 5)
        {
            this.route = $"{serviceUri}/api/Product";
            this.timeout = timeout * 1000;
        }

        public (Exception exception, Product product) Insert(ProductInsertDto product)
        {
            try
            {
                ...
                using (var br = new BinaryReader(response.GetResponseStream()))
                {
                    ...
                    var result = Product.ToConstruct(bytes);
                    return (null, result);
                }
            }
            catch (Exception ex)
        ...
        public (Exception exception, ProductQueryResponseDto response) Query(ProductQueryDto request)
        {
            try
            {
                ...
                {
                    byte[] bytes = br.ReadBytes((int)response.ContentLength);
                    var result = ProductQueryResponseDto.ToConstruct(bytes);
                    return (null, result);
                }
            }
            catch (Exception ex)
